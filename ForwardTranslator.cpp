...
//typedef boost::filesystem::basic_ifstream<char> IFSTREAM;
...
namespace radiance {
  ...
  // internal method used to format doubles as strings
	QVariantMap stadiccontrol;
	QVariantMap generalmap;
	QVariantMap spacemap;
	//QVariantList spaces;
	//QVariantMap thermalzonespacemap;

	QString jsonfile_spaces = QString::fromStdString("c:\\try3\\Data.json");
	QFile file_spaces(jsonfile_spaces);

	//openstudio::model::Model model;
	
	//  Open JSON file for output
	//QString jsonfile = "c:\\try3\\Data.json";
	//QFile file(jsonfile);
...
//	  boost::optional<model::DaylightingControl> primaryDaylightingControl = thermalZone.primaryDaylightingControl();
      ...
      if (!thermalZone){
		...
		spacemap.insert("Space without thermal zone removed", QString::fromStdString(space.name().get()));
        space.remove();
        continue;
      }
      ...
      for (openstudio::model::DaylightingControl daylightingControl : space.daylightingControls()){
        if (daylightingControl.isPrimaryDaylightingControl()){
			//QVariantMap thermalzonespacemap;
			//boost::optional<double> illumsetpoint = daylightingControl.illuminanceSetpoint();
			//std::string targetilluminance = toString(*illumsetpoint);
			//thermalzonespacemap.insert("target_illuminance", QString::fromStdString(targetilluminance));

          // ok
			//daylightingControl.PrimaryDaylightingControl();
	 ...
	 // System.Console.WriteLine("Hello!!");
	  //spaces << thermalzonespacemap;

	 // openstudio::model::DaylightingControl daylightingControl(m_model);
      std::vector<openstudio::model::DaylightingControl> daylightingControls = space.daylightingControls();
	std::vector<model::ThermalZone> thermalZones = model.getModelObjects<model::ThermalZone>();
	//QVariantMap thermalzonespacemap;
	//QVariantList thermalzonespacelist;

	if (thermalZones.size() > 0){
		for (model::ThermalZone thermalZone : thermalZones){

			
			//spacemap.insert("space_name_thermal_zone", QString::fromStdString( space.name().get()));
			boost::optional<model::DaylightingControl> primaryDaylightingControl = thermalZone.primaryDaylightingControl();
			//QVariantMap thermalzonespacemap;
			//QVariantList thermalzonespacelist;
			
			boost::optional<double> illuminancesetpoint = primaryDaylightingControl->illuminanceSetpoint();
			//std::string targetilluminance = toString(*illuminancesetpoint);
			//thermalzonespacemap.insert("target_illuminance", QString::fromStdString(targetilluminance));
			boost::optional<double> positionXCoordinate = primaryDaylightingControl->positionXCoordinate();
			boost::optional<double> positionYCoordinate = primaryDaylightingControl->positionYCoordinate();
			boost::optional<double> positionZCoordinate = primaryDaylightingControl->positionZCoordinate();
			boost::optional<double> psiRotationAroundXAxis = primaryDaylightingControl->psiRotationAroundXAxis();
			boost::optional<double> phiRotationAroundZAxis = primaryDaylightingControl->phiRotationAroundZAxis();
			boost::optional<double> thetaRotationAroundYAxis = primaryDaylightingControl->thetaRotationAroundYAxis();
			std::string lightingControlType = primaryDaylightingControl->lightingControlType();
			//boost::optional<double> minimumInputPowerFractionforContinuousDimmingControl = primaryDaylightingControl->minimumInputPowerFractionforContinuousDimmingControl();
			//boost::optional<double> minimumLightOutputFractionforContinuousDimmingControl = primaryDaylightingControl->minimumLightOutputFractionforContinuousDimmingControl();
			boost::optional<int> numberofSteppedControlSteps = primaryDaylightingControl->numberofSteppedControlSteps();
			//boost::optional<double> maximumAllowableDiscomfortGlareIndex = primaryDaylightingControl->maximumAllowableDiscomfortGlareIndex();
			//boost::optional<double> fractionofZoneControlledbyPrimaryDaylightingControl = thermalZone.fractionofZoneControlledbyPrimaryDaylightingControl();
			//boost::optional<double> fractionofZoneControlledbySecondaryDaylightingControl = thermalZone.fractionofZoneControlledbySecondaryDaylightingControl();
			//boost::optional<double> lightingPowerPerFloorArea = thermalZone.lightingPowerPerFloorArea();
			boost::optional<double> lightingPower = thermalZone.lightingPower();
				
			//thermalzonespacelist << thermalzonespacemap;
			//thermalzonespacelist << thermalzonespacemap;
			//spacemap.insert("illuminance setpoint", thermalzonespacelist);
//			spaces << spacemap;
		}
		//thermalzonespacelist << thermalzonespacemap;
		//thermalzonespacemap.insert("illuminance setpoint", thermalzonespacelist);
		//spaces << spacemap;
	...
	double northaxis = building.northAxis();

	// get epw file path and name
	...
	std::string epwF = toString(*epwPath);

	//inserting general values into json file	
	generalmap.insert("import_units", "m");
	generalmap.insert("illum_units", "lux");
	generalmap.insert("display_units", "m");
	generalmap.insert("daylight_savings_time", true);
	generalmap.insert("epw_file", QString::fromStdString(epwF));
	generalmap.insert("first_day", 1);
	generalmap.insert("building_rotation", northaxis);

	QVariantMap sda;
	QVariantList window_group_settings;
	sda.insert("calculate", false);
	sda.insert("illuminance", 300);
	sda.insert("DA_fraction", 0.5);
	sda.insert("start_time", 8);
	sda.insert("end_time", 18);
    window_group_settings << 1;
	sda.insert("window_group_settings", window_group_settings);

	//QVariantMap sda;
	//sda.insert("calculate", false);
	//sda.insert("illuminance", false);
	//sda.insert("DA_fraction", false);
	//sda.insert("start_time", false);
	//sda.insert("end_time", false);

	QVariantMap occupied_sDA;
	occupied_sDA.insert("calculate", false);
	occupied_sDA.insert("illuminance", 300);
	occupied_sDA.insert("DA_fraction", 0.5);

	//QVariantMap occupied_sDA;
	//occupied_sDA.insert("calculate", false);
	//occupied_sDA.insert("illuminance", false);
	//occupied_sDA.insert("DA_fraction", false);

	QVariantMap DA;
	DA.insert("calculate", true);
	DA.insert("illuminance", 300);

	QVariantMap cDA;
	cDA.insert("calculate", true);
	cDA.insert("illuminance", 300);

	QVariantMap DF;
	DF.insert("calculate", true);

	QVariantMap UDI;
	UDI.insert("calculate", true);
	UDI.insert("minimum", 100);
	UDI.insert("maximum", 3000);

	generalmap.insert("sDA", sda);
	generalmap.insert("occupied_sDA", occupied_sDA);
	generalmap.insert("DA", DA);
	generalmap.insert("cDA", cDA);
	generalmap.insert("DF", DF);
	generalmap.insert("UDI", UDI);

	//general << generalmap;

	

	  // openstudio::model::ThermalZone thermzone = space.thermalZone();
	   //boost::optional<openstudio::model::DaylightingControl> primarydaylightingcontrol
		  // = thermalZone.primaryDaylightingControl();
//	  std::vector<model::ThermalZone> thermalZones = modelObject.thermalZone();
	 // boost::optional<model::DaylightingControl> primarydaylightingcontrol = thermalZones.primaryDaylightingControl();
...
//	  std::vector<openstudio::model::DaylightingControl> DaylightingControl = modelObject.DaylightingControl();
	  //if (thermalZone){
		 // thermalZone->setPrimaryDaylightingControl(daylightingControl);
	 // }
	  //std::vector<model::ThermalZone_Impl> primaryDaylightingControl = thermalZone.primaryDaylightingControl();
	  //double illuminancesetpoint = thermalZone.PrimaryDaylightingControl.illuminanceSetpoint();
	//  boost::optional<DaylightingControl> primaryDaylightingControl = modelObject.primaryDaylightingControl();
  ...
  //boost::optional<IdfObject> ForwardTranslator::ForwardTranslateThermalZone(ThermalZone & modelObject)
  //{
	//  boost::optional<DaylightingControl> primaryDaylightingControl = modelObject.primaryDaylightingControl();
  //}
	   ...
	   std::vector<openstudio::path> &t_outfiles )
  {
	...
	//QVariantList glazing_materials;

	//std::vector<model::ThermalZone> thermalZones = m.getModelObjects<model::ThermalZone>();
	//model::ThermalZone thermalZone : thermalZones
...
//	std::vector<model::ThermalZone> thermalZones = model.getModelObjects<model::ThermalZone>();
	...
	for (const auto & space : t_spaces)
	{
		...
		QVariantList analysis_pts_files;
		QVariantMap analysis_pts;
		QVariantList window_groups;
		QVariantMap window_group_data;
		//inserting analysis point values into the json file
		std::string spacemapfile = space_name + ".map";
		analysis_pts_files << QString::fromStdString(spacemapfile);
		analysis_pts.insert("files", analysis_pts_files);
		...
		//inserting generic space data and directories into json file


		spacemap.insert("geometry_directory", "geometry/");
		spacemap.insert("ies_directory", "ies/");
		spacemap.insert("results_directory", "res/");
		spacemap.insert("input_directory", "data/");
		//spacemap.insert("glazing_materials", glazing_materials);

		//boost::optional<openstudio::model::ThermalZone> thermalZone = space.thermalZone();
		//spacemap.insert("thermal zone name", QString::fromStdString(thermalZone));
		//if (thermalZone)
		//{
			
			//std::vector<model::ThermalZone> thermalZones = m.getModelObjects<model::ThermalZone>();
			//spacemap.insert("enters the first thermal zone condition", "yeah!!");
			//if (thermalZones.size() > 0){
			
				//for (model::ThermalZone thermalZone : thermalZones)
				//{
				//	spacemap.insert("enters the  thermal zone loop", "yeah!!");
				//	boost::optional<model::DaylightingControl> primaryDaylightingControl = thermalZone.primaryDaylightingControl();
				//	boost::optional<double> illuminancesetpoint = primaryDaylightingControl->illuminanceSetpoint();
				//	std::string targetilluminance = toString(*illuminancesetpoint);
				//	spacemap.insert("target_illuminance", QString::fromStdString(targetilluminance));
	  ...
	  //if (thermalZones.size() > 0){
		 ...
		 // }
	  //}
		openstudio::path lightingschedulefile = t_radDir / openstudio::toPath("scene") / openstudio::toPath("lighting_schedule.csv");
		OFSTREAM lightingschedule(lightingschedulefile);
		int d, m, h;
		if (lightingschedule.is_open()){
			m = 1;
			while (m<=12)
			 {
				 if (m == 1 || 3 || 5 || 7 ||){
		...
		lightingschedule.close();
	  spacemap.insert("space_name", QString::fromStdString(space_name));
	  spacemap.insert("space_directory", "C:/OS1/ruby/TEST_3A/run/1-UserScript-0/radiance/");

	  spacemap.insert("ground_reflectance", 0.2);
	  spacemap.insert("material_file", "materials.rad");
	  spacemap.insert("geometry_file", "model_stadic.rad");
	  spacemap.insert("Lighting_schedule", "lighting_schedule.csv");
        ...
        for (const auto & subSurface : subSurfaces)
        {
              ...
              || subSurfaceUpCase == "SKYLIGHT")
          {
            ...
            if (control.outwardNormal){
				//double elevationazimuth = atan(double(control.outwardNormal->y() / control.outwardNormal->x()));
            ...
            //no shades
				// add materials
				window_group_data.insert("glazing_materials", QString::fromStdString(rMaterial));
			...
			else if (windowGroup_name == "WG1")
			{
				rMaterial = "trans";
				m_radMaterials.insert("void " + rMaterial + " glaz_" + rMaterial + "_tn-" + formatString(tn, 3) + "\n" + matString + "\n");
				m_radMaterialsDC.insert("void alias glaz_" + rMaterial + "_tn-" + formatString(tn, 3) + " WG1\n\n");
				m_radMaterialsSwitchableBase.insert("void alias glaz_" + rMaterial + "_tn-" + formatString(tn, 3) + " WG1\n\n");
                ...
                formatString(vertex->z()) + "\n";
							...
							if (shadingControl){
								if (shadingControl->construction()){
									...
									m_radMaterialsWG1.insert("void trans " + windowGroup_name + "\n0\n0\n5\n0 0 0 0 0\n");
									...
									m_radMaterialsWG1.insert("void trans " + windowGroup_name + "_SHADE\n0\n0\n5\n0 0 0 0 0\n");
	  ...
	  // boost::optional<openstudio::model::ThermalZone> thermalZone = space.thermalZone();
	 //  boost::optional<double> fractionofZoneControlledbyPrimaryDaylightingControl = thermalZone->fractionofZoneControlledbyPrimaryDaylightingControl();
	  // std::string fractionofZoneControlledbyPrimaryDaylightControl = toString(*fractionofZoneControlledbyPrimaryDaylightingControl);
	 //  control_zone_data.insert("fraction_of_Zone_Controlled_by_Primary_Daylighting_Control", QString::fromStdString(fractionofZoneControlledbyPrimaryDaylightControl));
	  ...
	  //for (model::ThermalZone thermalZone : thermalZones){
	   ...
	   //
		...
		//  boost::optional<double> fractionofZoneControlledbySecondaryDaylightingControl = thermalZone.fractionofZoneControlledbySecondaryDaylightingControl();
		//  std::string fractionofZoneControlledbySecondaryDaylightControl = toString(*fractionofZoneControlledbySecondaryDaylightingControl);
		//  control_zone_data.insert("fraction_of_Zone_Controlled_by_Secondary_Daylighting_Control", QString::fromStdString(fractionofZoneControlledbySecondaryDaylightControl));
	 ...
	 // }
      ...
      {
		  QVariantList control_zones;
		  QVariantMap control_zone_data;
        m_radSensors[space_name] = "";

        openstudio::Point3d sensor_point = openstudio::radiance::ForwardTranslator::getReferencePoint(control);
		//inserting control zone data into json file
		control_zone_data.insert("name", "zone1");
		control_zone_data.insert("optimum_control", "Eplus_dim_to_min");
		QVariantMap sensor;
		QVariantMap location;
		//location.insert("x:", QString::fromStdString(formatString(sensor_point.x(), 3)));
		location.insert("x", sensor_point.y());
		//location.insert("y:", QString::fromStdString(formatString(sensor_point.y(), 3)));
		location.insert("y", sensor_point.y());
		//location.insert("z:", QString::fromStdString(formatString(sensor_point.z(), 3)));
		location.insert("z", sensor_point.z());
		location.insert("xd", 0);
		location.insert("yd", 0);
		location.insert("zd", 1);
		location.insert("spin_ccw", 0);


		sensor.insert("sensor_type", "cosine");
		sensor.insert("location", location);
		control_zone_data.insert("sensor", sensor);
		control_zone_data.insert("lighting_power", 100);
		//boost::optional<double> fractionofZoneControlledbyPrimaryDaylightingControl = control.fractionofZoneControlledbyPrimaryDaylightingControl();
		//std::string fractionofZoneControlledbyPrimaryDaylightControl = toString(*fractionofZoneControlledbyPrimaryDaylightingControl); 
		//control_zone_data.insert("fraction_of_Zone_Controlled_by_Primary_Daylighting_Control", QString::fromStdString(fractionofZoneControlledbyPrimaryDaylightControl));
		
		//boost::optional<double> fractionofZoneControlledbySecondaryDaylightingControl = control.fractionofZoneControlledbySecondaryDaylightingControl();
		//std::string fractionofZoneControlledbySecondaryDaylightControl = toString(*fractionofZoneControlledbySecondaryDaylightingControl);
		//control_zone_data.insert("fraction_of_Zone_Controlled_by_Secondary_Daylighting_Control", QString::fromStdString(fractionofZoneControlledbySecondaryDaylightControl));

		double illumsetpoint = control.illuminanceSetpoint();
		//std::string targetilluminance = toString(*illumsetpoint);
		spacemap.insert("target_illuminance", illumsetpoint);

		QVariantMap luminaire_information;
		luminaire_information.insert("ies_file", "null");
		control_zone_data.insert("luminaire_information", luminaire_information);
		control_zone_data.insert("cp_method", "Eplus_auto");

		QVariantMap ballast_driver_information;

		ballast_driver_information.insert("ballast_type", "linear_dimming");

		double minimumInputPowerFractionforContinuousDimmingControl = control.minimumInputPowerFractionforContinuousDimmingControl();
		std::string mininputpowerfractionforcontinuousdimmingcontrol = toString(minimumInputPowerFractionforContinuousDimmingControl);
		//ballast_driver_information.insert("bf_max", QString::fromStdString(mininputpowerfractionforcontinuousdimmingcontrol));
		ballast_driver_information.insert("bf_max", 1.0);

		double minimumLightOutputFractionforContinuousDimmingControl = control.minimumLightOutputFractionforContinuousDimmingControl();
		//std::string minoutputpowerfractionforcontinuousdimmingcontrol = toString(minimumLightOutputFractionforContinuousDimmingControl);
		//ballast_driver_information.insert("bf_min", QString::fromStdString(minoutputpowerfractionforcontinuousdimmingcontrol));
		ballast_driver_information.insert("bf_min", minimumLightOutputFractionforContinuousDimmingControl);

		boost::optional<openstudio::model::ThermalZone> thermalZone = space.thermalZone();
		//if (thermalZone.size() > 0){
//		for (model::ThermalZone thermalZone : thermalZones){
				double fractionofZoneControlledbyPrimaryDaylightingControl = thermalZone->fractionofZoneControlledbyPrimaryDaylightingControl();
				//std::string fractionofZoneControlledbyPrimaryDaylightControl = toString(*fractionofZoneControlledbyPrimaryDaylightingControl);
				control_zone_data.insert("fraction_controlled", fractionofZoneControlledbyPrimaryDaylightingControl);

				//boost::optional<double> fractionofZoneControlledbySecondaryDaylightingControl = thermalZone->fractionofZoneControlledbySecondaryDaylightingControl();
				//std::string fractionofZoneControlledbySecondaryDaylightControl = toString(*fractionofZoneControlledbySecondaryDaylightingControl);
				//control_zone_data.insert("fraction_of_Zone_Controlled_by_Secondary_Daylighting_Control", QString::fromStdString(fractionofZoneControlledbySecondaryDaylightControl));

				double lightingPowerPerFloorArea = thermalZone->lightingPowerPerFloorArea();
				//std::string wattsperspacefloorarea = toString(lightingPowerPerFloorArea);
				ballast_driver_information.insert("watts_max", lightingPowerPerFloorArea);

				double watts_min = lightingPowerPerFloorArea * minimumInputPowerFractionforContinuousDimmingControl;

				//std::string min_watts = toString(watts_min);
				ballast_driver_information.insert("watts_min", watts_min);

				control_zone_data.insert("ballast_driver_information", ballast_driver_information);
			//}
		//}
		control_zones << control_zone_data;
		spacemap.insert("control_zones", control_zones);
      ...
      OFSTREAM file(filename);
	  openstudio::path modelfilename = t_radDir / openstudio::toPath("scene") / openstudio::toPath("model_stadic.rad");
	  OFSTREAM modelfile(modelfilename, std::ofstream::out | std::ofstream::app);
      if (file.is_open()){
        t_outfiles.push_back(filename);
        m_radSceneFiles.push_back(filename);
		...
		if (modelfile.is_open()){
			...
			m_radSceneFiles.push_back(modelfilename);
			modelfile << m_radSpaces[space_name];
		}
		//else
		//{
...
//		  LOG(Error, "Cannot open file '" << toString(modelfilename) << "' for writing");
	//  }
      ...
      { 
		  //QVariantList window_groups;
		  //QVariantMap window_group_data;
		  QVariantList angle_settings;
		  QVariantList shade_settings;
		  QVariantList calculate_settings;
		  
		  //inserting window group data into the json file
		  std::string windowGroup_name = windowGroup.name();
		 // if (windowGroup_name == "WG0"){
		  ...
		  //}
			  ...
			  window_group_data.insert("name", QString::fromStdString(windowGroup_name));
			  //window_group_data.insert("base_geometry", "empty.rad");
			  window_group_data.insert("base_geometry", "WG0.rad");
			  window_group_data.insert("calculate_base", true);
			  WindowGroupControl windowgroupcontrol = windowGroup.windowGroupControl();
			  double elevationazimuth = ((-(atan(double(windowgroupcontrol.outwardNormal->x() / -(windowgroupcontrol.outwardNormal->y()))))) * 180) / 3.14159265;
			 QVariantMap shade_control;
			 double apa = 45.0;
			 std::string automatedprofileangle = formatString(apa, 3);

			  //shade_control.insert("method", QString::fromStdString(automatedprofileangle));
			 shade_control.insert("method", "automated_profile_angle");
			 shade_control.insert("elevation_azimuth", elevationazimuth);

			angle_settings << 45;
			 shade_control.insert("angle_settings", angle_settings);

			  //shade_settings << "WG0.rad";
			 shade_settings << "WG1.rad";

			 calculate_settings << true;
			 window_group_data.insert("shade_control", shade_control);
			 window_group_data.insert("shade_settings", shade_settings);
			 window_group_data.insert("calculate_setting", calculate_settings);

			 window_groups << window_group_data;
			  spacemap.insert("window_groups", window_groups);
		  //}
	  ...
	  spacemap.insert("analysis_points", analysis_pts);
	  ...
	  // write radiance WG1 vmx materials file (blacks out controlled window groups)
	  m_radMaterialsWG1.insert("# OpenStudio \"WG1\" Materials File\n# black out all controlled window groups.\n");
	  openstudio::path materials_WG1filename = t_radDir / openstudio::toPath("materials/materials_WG1.rad");
	  OFSTREAM materials_WG1file(materials_WG1filename);
	  if (materials_WG1file.is_open()){
		  t_outfiles.push_back(materials_WG1filename);
		  for (const auto & line : m_radMaterialsWG1)
		  {
			  materials_WG1file << line;
		  };
	  }
	  else{
		  LOG(Error, "Cannot open file '" << toString(materials_WG1filename) << "' for writing");
	  }
      ...
      //  {
		...
		//	std::ifstream spacefile(filename, std::ios_base::binary);
			//OFSTREAM spacefile(filename);
			//spacefile.open();
		//	if (spacefile.fail()){ //check for failure to open
		//		LOG(Error, "Cannot open file '" << toString(filename) << "' for copying");
		//	}
     ...
     // }
	 // QVariantMap sda;
	  //sda.insert("calculate", true);
	 // sda.insert("illuminance", 300);
	 // sda.insert("DA_fraction", 0.5);
	  //sda.insert("start_time", 8);
	 // sda.insert("end_time", 17);

	 // QVariantMap occupied_sDA;
	//  occupied_sDA.insert("calculate", true);
	//  occupied_sDA.insert("illuminance", 300);
	//  occupied_sDA.insert("DA_fraction", 0.5);
	...
	//  UDI.insert("calculate", true);
	//  UDI.insert("minimum", 100);
	//  UDI.insert("maximum", 250);
	//  spacemap.insert("sDA", sda);
	//  spacemap.insert("occupied_sDA", occupied_sDA);
	//  spacemap.insert("DA", DA);
	//  spacemap.insert("cDA", cDA);
	//  spacemap.insert("DF", DF);
	//  spacemap.insert("UDI", UDI);
//	  spacemap.insert("general", general);

	  spaces << spacemap;

	  //stadiccontrol.insert("spaces", spaces);

	  

	    //Write out JSON file
	 // QJsonObject jsonnew = QJsonObject::fromVariantMap(stadiccontrol);
	  //QJsonDocument jsondoc(jsonnew);
	  
	  //if (file_spaces.open(QIODevice::Append)) {
		// QTextStream stream(&file_spaces);
		 // stream << jsondoc.toJson() << endl;
	 // }

    }
	//printing out the json file
	stadiccontrol.insert("general", generalmap);
	stadiccontrol.insert("spaces", spaces);
	QJsonObject jsonnew = QJsonObject::fromVariantMap(stadiccontrol);
	QJsonDocument jsondoc(jsonnew);
